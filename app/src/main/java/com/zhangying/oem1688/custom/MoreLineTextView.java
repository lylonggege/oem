...
public class MoreLineTextView extends LinearLayout implements View.OnClickListener {
    ...
    private void initWithAttrs(AttributeSet attrs) {
        if (isInEditMode()) {
            ...
            mTextColor = typedArray.getColor(R.styleable.MoreTextStyle_textColor1, ContextCompat.getColor(getContext(), R.color.b3));
            mTextSize = typedArray.getDimension(R.styleable.MoreTextStyle_textSize1, ScreenTools.instance(context).dip2px(14));
            ...
            if (mExpandDrawable == null) {
                mExpandDrawable = ContextCompat.getDrawable(getContext(), R.drawable.icon_backtop);
            }
            typedArray.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 内容区域初始显示行高
        mTvContent.setHeight(mTvContent.getLineHeight() * (mMaxLine > mTvContent.getLineCount() ? mTvContent.getLineCount() : mMaxLine) + 40);
    ...
    public void onClick(View v) {
        ...
        Animation animation = new Animation() {
            protected void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                mTvContent.setHeight((int) (startValue + deltaValue * interpolatedTime + (isExpand ? 50 : 0)));
            }
        };
        animation.setDuration(mDurationMillis);
