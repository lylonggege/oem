...
public class WeiBoContentTextUtil {
    ...
    private  TopicOrAtClickListener topicOrAtClickListener;

    private String otherClickStr;
    private boolean isOtherClick;
    private int otherClickStrStart;
    private int otherClickStrEnd;

    private static class Section {
        static final int AT = 1;
        static final int TOPIC = 2;// ##话题
        static final int LINK = 3;// ##链接
    ...
    private static final String LINK = "([hH][tT]{2}[pP]://|[hH][tT]{2}[pP][sS]://|[wW]{3}.|[wW][aA][pP].|[fF][tT][pP].|[fF][iI][lL][eE].)[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]";// ##链接

    private static final String ALL = "(" + AT + ")" + "|" + "(" + TOPIC + ")" +"|"+"("+LINK+")";

    public SpannableStringBuilder getWeiBoContent(final String source, final Context context, final TextView textView, String foregroundColor, String backgroundColor) {
        ...
        while (matcher.find()) {
            ...
            final String link = matcher.group(3);
            ...
            if (at != null) {
                ...
                ForegroundColorSpan foregroundColorSpan = new ForegroundColorSpan(Color.parseColor(foregroundColor));
            ...
            if(link!=null){
                ...
                Section section = new Section(start,end, Section.LINK,link);
                sections.add(section);
                ForegroundColorSpan foregroundColorSpan = new ForegroundColorSpan(Color.parseColor(foregroundColor));
                spannableStringBuilder.setSpan(foregroundColorSpan, start, end, Spannable.SPAN_INCLUSIVE_INCLUSIVE);
            }
        ...
        final BackgroundColorSpan span = new BackgroundColorSpan(Color.parseColor(backgroundColor));
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                switch(action) {
                    case MotionEvent.ACTION_DOWN://TODO 最后一行点击问题 网址链接
                        ...
                        if(isOtherClick){
                            if(index >= otherClickStrStart && index<= otherClickStrEnd) {
                                spannableStringBuilder.setSpan(span, otherClickStrStart, otherClickStrEnd, Spannable.SPAN_INCLUSIVE_INCLUSIVE);
                                textView.setText(spannableStringBuilder);
                                textView.getParent().requestDisallowInterceptTouchEvent(true);//不允许父view拦截
                                return true;
                            }
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        Log.d(TAG, "ACTION_UP,x:"+upX+",y:"+upY+spannableStringBuilder.toString());
                        if (Math.abs(upX-downX) < slop && Math.abs(upY-downY) < slop) {
                            //TODO startActivity or whatever
                            if (downSection != null) {
                                String name = downSection.name;
                                if(topicOrAtClickListener!=null){
                                    topicOrAtClickListener.topicOrAtClick(name,downSection.type);
                                }
                                downSection = null;
                            }else {
                                if(isOtherClick){
                                    if(topicOrAtClickListener!=null){
                                        topicOrAtClickListener.topicOrAtClick(otherClickStr,4);
                                    }
                                    return true;
                                }
                                return false;
    ...
    public interface TopicOrAtClickListener{
        void topicOrAtClick(String name , int clickType);
    }

    public  void setTopicOrAtClickListener(TopicOrAtClickListener topicOrAtClickListener){
        this.topicOrAtClickListener = topicOrAtClickListener;
    }


    public void setClick(boolean isOtherClick , String otherClickStr , int otherClickStrStart , int otherClickStrEnd){
        this.isOtherClick = isOtherClick;
        this.otherClickStr = otherClickStr;
        this.otherClickStrStart = otherClickStrStart;
        this.otherClickStrEnd = otherClickStrEnd;
    }
}
